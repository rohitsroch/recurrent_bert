...
flags.DEFINE_integer("max_predictions_per_seq", 77,
...
def create_instances_from_document(
    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                # `a_end` is how many segments from `current_chunk`
                # go into the `A` (first) sentence.
                a_end = 1
                ...
                if len(current_chunk) == 1 or rng.random() < 0.5:
                    ...
                    # large corpora. However, just to be careful, we try
                    # to make sure that the random document is not the
...
def create_masked_lm_predictions(tokens, masked_lm_prob,
    ...
    for (i, token) in enumerate(tokens):
        ...
        # subsequence tokens are prefixed with ##. So whenever we
        # see the ## token, we append it to the previous set of word indexes.
        #

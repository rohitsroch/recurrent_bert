...
class BertModel(object):
    ...
    def __init__(self,
        ...
        with tf.compat.v1.variable_scope(scope, default_name="bert"):
            ...
            # (or segment-pair-level) classification tasks where we need a
            # fixed dimensional representation of the segment.
            with tf.compat.v1.variable_scope("pooler"):
                # We "pool" the model by simply taking the hidden state
                # corresponding to the first token. We assume that this
                ...
                self.pooled_output = tf.compat.v1.layers.dense(
                    ...
                    config.hidden_size,
                    activation=tf.tanh,
                    kernel_initializer=create_initializer(config.
                                                          initializer_range))
    ...
    def get_embedding_output(self):
        ...
        Returns:
          float Tensor of shape [batch_size, seq_length, hidden_size]
          corresponding to the output of the embedding layer,
          after summing the word embeddings with the positional embeddings
          and the token type embeddings, then performing layer normalization.
...
def get_activation(activation_string):
    ...
    Returns:
      ...
      it will return `activation_string`.
...
def dropout(input_tensor, dropout_prob):
    ...
    Args:
      ...
      dropout_prob: Python float. The probability of dropping
         out a value (NOT of *keeping* a dimension as in `tf.nn.dropout`).
...
def embedding_postprocessor(input_tensor,
    ...
    Args:
      ...
      token_type_embedding_name: string. The name of the
        embedding table variable for token type ids.
      ...
      applied to the final output tensor.
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            full_position_embeddings = tf.compat.v1.get_variable(
                ...
                initializer=create_initializer(initializer_range))
            # Since the position embedding table is a learned variable,
            # we create it using a (long) sequence length
            # `max_position_embeddings`. The actual sequence length might
            # be shorter than this, for faster training of
            # tasks that do not have long sequences.
            #
            # So `full_position_embeddings` is effectively an embedding table
            # for position [0, 1, 2, ..., max_position_embeddings-1],
            # and the current sequence has positions
            ...
            # (`seq_length` and `width`), so we broadcast among the first
            # dimensions, which is typically just
            # the batch size.
...
def create_attention_mask_from_input_mask(from_tensor, to_mask):
    ...
    # don't actually care if we attend *from*
    # padding tokens (only *to* padding)
...
def attention_layer(from_tensor,
    ...
    Args:
      ...
      attention_mask: (optional) int32 Tensor of shape [batch_size,
        ...
        attention scores will effectively be set to -infinity for any
        positions in the mask that are 0, and will be unchanged
      ...
      do_return_2d_tensor: bool. If True, the output will be of shape
        ...
        If False, the output will be of shape
        [batch_size, from_seq_length, num_attention_heads s* size_per_head].
      ...
      to_seq_length: (Optional) If the input is 2D, this might be the seq
        length of the 3D version of the `to_tensor`.
    ...
    if attention_mask is not None:
        ...
        # tensor which is 0.0 for positions we want to attend and -10000.0
...
def transformer_model(input_tensor,
    ...
    Args:
      ...
      intermediate_size: int. The size of the "intermediate" (a.k.a., feed
        forward) layer.
      intermediate_act_fn: function. The non-linear activation
        function to apply to the output of the intermediate/feed-forward layer.
    ...
    if input_width != hidden_size:
        raise ValueError("The width of the input tensor \
                          (%d) != hidden size (%d)" %
                         (input_width, hidden_size))

    # We keep the representation as a 2D tensor to avoid re-shaping it back and
    # forth from a 3D tensor to a 2D tensor. Re-shapes are normally free on
    # the GPU/CPU but may not be free on the TPU, so we want to minimize them
        ...
        with tf.compat.v1.variable_scope("layer_%d" % layer_idx):
            ...
            with tf.compat.v1.variable_scope("attention"):
                ...
                if len(attention_heads) == 1:
                    ...
                    # In the case where we have other sequences, we just
                    # concatenate them to the self-attention head before
                    # the projection.
